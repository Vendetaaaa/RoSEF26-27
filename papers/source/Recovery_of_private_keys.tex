...
\texttt{ecdsa\_leakage\_model4.py} produces synthetic HW/HD traces with Gaussian noise
\[
\sigma\in\{0.25,0.50,1.00,1.50,2.00,3.00\}.
\]
For each noise level, the project reports the accuracy of a linear decoder and the signal-to-noise ratio, providing a baseline for the neural network.
...
\texttt{hnp\_attack1.py} and \texttt{Lattice\_key6.py} construct the HNP instance from estimated prefixes or, for implementation validation, from ground truth using the \texttt{--source oracle} option. The code reduces the lattice with LLL, extracts a private-key candidate, and validates it independently.
...
The automated test suite, including \texttt{tests/test\_math.py}, \texttt{test\_model.py}, \texttt{test\_simulator.py}, and \texttt{test\_smoke.py}, runs through continuous integration workflows. The tests verify curve arithmetic, the modules in Section 5, network training, and pipeline reproducibility for the fixed seed
\[
...
In the experiments, nonce-bit decoding accuracy decreases as $\sigma$ increases. The success of LLL reduction depends on the error bound $B$ and the number of signatures $m$. When $B$ increases, more solutions become compatible because fewer nonce bits are known. Increasing $m$ adds constraints and changes the lattice dimension, affecting the relation between the target vector and typical vectors in the reduced basis.
...
\item evaluate CNN accuracy and LLL success rate quantitatively as a function of $(\ell,\sigma,m)$, with results published together with the source code.
\end{enumerate}
